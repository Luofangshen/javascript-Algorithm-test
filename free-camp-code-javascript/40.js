...
2152.52
写到一半，还是思考不出解法 过几天在考虑下

 */

function checkCashRegister(price, cash, cid) {
  let checkCash = cash - price
  let bai = Math.floor(checkCash / 100)
  let shi = Math.floor(checkCash / 10) % 10
  let ge = Math.floor(checkCash) % 10
  let pointOne = Math.floor(checkCash * 10) % 10
  let pointTwo = Math.floor(checkCash * 100) % 10
  let newArr = []
  console.log(bai, shi, ge, pointOne, pointTwo)
  cid = cid.reverse()
  cid.forEach((item, index) => {
    if (item[1] > bai * 100) {
      item[1] = bai * 100
      newArr.push(item)
    } else if (item[1] > shi * 10) {
